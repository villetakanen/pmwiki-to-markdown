...
describe("convertSmallText", () => {
  it("should convert PMWiki small text markup to HTML span tags", () => {
    ...
    const expectedMarkdown = '<span class="text-small"> _Amiri, human barbarian._ </span>';
  ...
  it("should not convert similar patterns that are not small text", () => {
    const wikitext = "This is [not small] text and some text [- properly -] formed";
    const expectedMarkdown = "This is [not small] text and some text <span class=\"text-small\"> properly </span> formed";
    expect(convertSmallText(wikitext)).toBe(expectedMarkdown);
  });
});
